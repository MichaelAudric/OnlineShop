...
const Cart = () => {
  ...
  useEffect(() => {
    if (!userID) return;
    Axios.get(`https://online-shop.vercel.app/api/cart/${userID}`)
      .then((response) => {
  ...
  const handleRemoveItem = (productID) => {
    Axios.delete(
      `https://online-shop.vercel.app/api/cart/${userID}/${productID}`
    )
      .then(() => {
  ...
  const handlePay = () => {
    Axios.post(`https://online-shop.vercel.app/api/cart/pay/${userID}`)
      .then((response) => {
